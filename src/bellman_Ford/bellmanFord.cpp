...
int main(int argc, char **argv){
    ...
    if((strcmp(it->first, "-l") == 0)){
        if(writeFile){            
            bellmanFord(G, first, second, itFWrite -> second);
        }else{
            bellmanFord(G, first, second);
        }
    }else{
        if(writeFile){
            bellmanFord(G, first, itFWrite -> second);
        }else{
            bellmanFord(G, first);
        }
    }
...
void showHelp(){
    std::cout <<
    "Implementacao do algoritmo de Bellman-Ford por Joao Victor Araujo\nversao 1.0\nPara utilizar o programa voce "
    ...
    <<"4)\tbellmanFord -i 1 -f entrada.txt -l 4\nsaida:\t10\n\n"
    <<"5)\tbellmanFord -i 1 -l 4 -f entrada.txt -s\nsaida:\t10\nNo caso 5 o comando \"-s\" fora utilizado junto do "
    ...
    << "\"bellmanFord -i -s 1 -l -f entrada.txt 4\" nao sera executado propriamente";
};
