...
int main(int argc, char **argv){
    ...
    bool readFile = false, writeFile = false;
    std::ifstream fRead;
    ...
    if((strcmp(it->first, "-l") == 0)){
        if(writeFile){            
            dijkstra(G, first, second, itFWrite -> second);
        }else{
            dijkstra(G, first, second);
        }
    }else{
        if(writeFile){
            dijkstra(G, first, itFWrite -> second);
        }else{
            dijkstra(G, first);
        }
    }
...
void showHelp(){
    std::cout <<
    "Implementacao do algoritmo de Dijkstra por Joao Victor Araujo\nversao 1.0\nPara utilizar o programa voce "
    ...
    <<"\n\t-l : vertice final\n\n"
    <<"EXEMPLOS DE EXECUCAO:\n1)\t dijkstra -i 1 -l 4 -f entrada.txt -o saida.txt\n"
    <<"saida:\tO resultado do programa foi salvo no arquivo \"saida.txt\"\n\n"
    <<"2)\tdijkstra -i 1 -f entrada.txt -s\nsaida:\t1:0 2:3 3:4 4:2 5:10 6:3\n\n"
    ...
    <<"nao conseguira ler as entradas para construir o grafo.\n\n"
    <<"4)\tdijkstra -i 1 -f entrada.txt -l 4\nsaida:\t10\n\n"
    <<"5)\tdijkstra -i 1 -l 4 -f entrada.txt -s\nsaida:\t10\nNo caso 5 o comando \"-s\" fora utilizado junto do "
    <<"comando \"-l\", sendo que o comando \"-s\" gera a solucao completa do problema na saida, enquanto que "
    <<"o comando \"-l\" restringe a solucao da saida ser a distancia entre o vertice passado juntamente com "
    ...
    << "\"dijkstra -i -s 1 -l -f entrada.txt 4\" nao sera executado propriamente";
}
